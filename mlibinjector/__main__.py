...
def main():
	...
	parser.add_option('-R', action='store_true', dest='recursive', help='Recursively check for APK to decompile in the given folder')
	...
	if v.recursive and os.path.isdir(apkname):
		for x in glob(apkname + '/**/*.apk', recursive=True):
			inject(x, v)
	else:
		inject(apkname, v)
